...
from src.core.models.grad_cam import GradCAM 
from src.core.models.grad_cam_plusplus import GradCAMPlusPlus
from src.core.models.score_cam import ScoreCAM
from src.core.models.image import (
    show_cam_on_image, deprocess_image, preprocess_image
)
from src.core.models.guided_backprop import GuidedBackpropReLUModel

from src.core.models.cam import get_args
from src.core.models.vgg import vgg16
from src.core.models.resnet50_no import resnet50
from src.core.models.resnet import resnet5
from pytorch_pretrained_vit import ViT
#from src.core.models.senet import legacy_seresnet50
...
class Engine(BaseEngine):
    ...
    def __init__(self, mode, config_path, log, train_dir, eval_dir=None):
        ...
        # Build a model(that i changed)

        #if you want resnet50, try this code
        #self.model = resnet5(pretrained=True)
        #self.model = legacy_seresnet50(pretrained=True)

        self.model = ViT('B_16_imagenet1k', pretrained=True)
        #if you want vgg16, try this code
        # num_classes_cub = 200
        # self.model = vgg16(pretrained=True)
        # self.model.classifier[6] = torch.nn.Linear(4096, num_classes_cub)
        

        ##if you want resnet50, try this code
        #num_classes_cub = 200
        #self.model.fc = torch.nn.Linear(2048, num_classes_cub)
        ...
        # }
        # torch.save(checkpoint, '/Users/shinminkyoung/Downloads/rethinkingCAM_plus/configs/weight.pt')

        # checkpoint = torch.load('/Users/shinminkyoung/Downloads/rethinkingCAM_plus/configs/weight.pt')

        # # 모델의 가중치 로드
        # self.model.load_state_dict(checkpoint['model_state_dict'])


        #torch.save(self.model.state_dict(), '/Users/shinminkyoung/Downloads/rethinkingCAM_plus/configs/weight.pt')
    ...
    def _train_one_epoch(self, epoch, num_step, next_checkpoint_step):
        ...
        for i, (images, labels, _) in enumerate(self.dataloader):
            ...
            losses = self.criterion(**resultss)
            loss = losses['loss']
    ...
    def _eval_one_epoch(self, epoch, num_step, iou_threshold, loc_threshold=0.2,
        ...
        for i, (images, labels, gt_boxes) in enumerate(self.dataloader):
            ...
            # Class activation map-gradcam & gradcam_plusplus
            # with grad_cam(model=self.model, target_layers=target_layers,use_cuda=True) as cam:
                
            #     gt_cams, outputs = cam(input_tensor=images, targets=targets,aug_smooth=True, eigen_smooth=True)

            # with grad_cam_plusplus(model=self.model, target_layers=target_layers,use_cuda=True) as cam:
                
            #     gt_cams, outputs = cam(input_tensor=images, targets=targets,aug_smooth=True, eigen_smooth=True)

            # with score_cam(model=self.model, target_layers=target_layers,use_cuda=True) as cam:
                
            #     gt_cams, outputs = cam(input_tensor=images, targets=targets,aug_smooth=True, eigen_smooth=True)


            
            # print('gt_cams',gt_cams.shape)
            # print('gt_cams',type(gt_cams))
            gt_cams = torch.tensor(gt_cams)
            gt_cams = gt_cams.unsqueeze(1)

            # Compute Top-1 and Top-5 Cls
            batch_size = images.size(0)
            predictions = outputs #results['preds']
